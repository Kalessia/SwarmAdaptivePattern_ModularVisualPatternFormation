...
class NeuralNetwork:
    ...
    def setWeightsFromList(self, tabWeights):
        ...
        for layer in range(self.n_hiddenLayers):
            ...
            weightsDict[s] = []
            for neuron in range(self.n_neuronsPerHidden):
                weightsDict[s].append([tabW.pop(0) for i in range(self.n_neuronsPreviousLayer)])
            self.n_neuronsPreviousLayer = self.n_neuronsPerHidden
        ...
        for neuron in range(self.n_neuronsPerOutputs):
            if self.n_hiddenLayers > 0 :   
                weightsDict[s].append([tabW.pop(0) for i in range(self.n_neuronsPerHidden)])
            else:
                weightsDict[s].append([tabW.pop(0) for i in range(self.n_neuronsPerInputs + 1)])

        self.weights = weightsDict

        # print("modified weights", self.weights)
    ...
    def neuronActivation(self, weightsToPreviousLayer, inputsPreviousLayer):
        ...
        # print("blabla", weightsToPreviousLayer, inputsPreviousLayer)

        # activation = sum (neuron_i * weight_i + biais)
        for i in range(len(weightsToPreviousLayer)-1):
            # print("bllllu", inputsPreviousLayer[i], weightsToPreviousLayer[i])
    ...
    def stableSigmoid(self, x):
        """
        Returns the result of the function sigmoid(x) avoiding overflow numeric issue
        """
        if x >= 0:
            z = math.exp(-x)
            sig = 1.0 / (1.0 + z)
