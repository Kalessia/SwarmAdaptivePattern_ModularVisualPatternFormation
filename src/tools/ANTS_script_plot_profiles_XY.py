...
for index in row_indexes_to_inspect:
    ...
    plt.plot(flagSingleEnv_x_values, label='Best flag singleEnv step 49', color=colors['flagSingleEnv'], linestyle=line_styles['flagSingleEnv'], linewidth=1)

    # Flag multiEnv
    flagMultiEnv_x_values = flagMultiEnv_array[index, :, 0]
    plt.plot(flagMultiEnv_x_values, label='Best flag multiEnv step 49', color=colors['flagMultiEnv'], linestyle=line_styles['flagMultiEnv'], linewidth=1)
    ...
    plt.xlabel("Grid column index", fontsize=18, labelpad=20)
    plt.ylabel("X value", fontsize=18, labelpad=20)
...
for index in cols_indexes_to_inspect:
    ...
    plt.plot(flagSingleEnv_y_values, label='Best flag singleEnv step 49', color=colors['flagSingleEnv'], linestyle=line_styles['flagSingleEnv'], linewidth=1)

    # Flag multiEnv
    flagMultiEnv_y_values = flagMultiEnv_array[:, index, 1]
    plt.plot(flagMultiEnv_y_values, label='Best flag multiEnv step 49', color=colors['flagMultiEnv'], linestyle=line_styles['flagMultiEnv'], linewidth=1)

    plt.suptitle(f"Profile XY - Column {index}, component Y", fontsize=18)
    plt.title("Pattern: coordinates 16x16; 20 runs; 280,000 evaluations", fontsize=18)
    plt.xlabel("Grid row index", fontsize=18, labelpad=20)
    plt.ylabel("Y value", fontsize=18, labelpad=20)
    plt.xticks(fontsize=16)
    plt.yticks(fontsize=16)
    plt.xlim(0, 15)
    plt.ylim(-0.01, 1.01)
